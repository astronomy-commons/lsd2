...
def _gather_statistics_hpix_hist(parts, k, cache_dir, fmt, ra_kw, dec_kw):
    ...
    for fn in parts:
        parqFn = os.path.join(
            cache_dir, os.path.basename(fn).split(".")[0] + ".parquet"
        )
        if not os.path.exists(parqFn):
            ...
            if "csv" in fmt:
                ...
                sys.exit(
                    f"File Format: {fmt} not implemented! \
...
def _write_partition_structure(
    url, cache_dir, output_dir, orders, opix, ra_kw, dec_kw, id_kw
):
    ...
    for k in orders:
        ...
        ret = order_df.groupby(["hips_k", "hips_pix"]).apply(
            _to_hips, hipsPath=output_dir, base_filename=base_filename
        )
...
def _cross_match(match_cats, c1_md, c2_md, n_neighbors=1, dthresh=4.0):
    ...
    for id1, ra1, dec1 in zip(
        c1_df[c1_md["id_kw"]], c1_df[c1_md["ra_kw"]], c1_df[c1_md["dec_kw"]]
    ):

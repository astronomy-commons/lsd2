...
class Catalog:
    def __init__(
        self, catname="gaia", source="local", location="/epyc/projects3/sam_hipscat/"
    ):
        ...
        if self.source == "local":
            ...
            if not os.path.exists(self.output_dir):
                print(
                    "No local hierarchal catalog exists, run catalog.hips_import(file_source='/path/to/file_or_files', fmt='csv.gz')"
                )
            else:
    ...
    def hips_import(
        ...
        if len(urls):
            self.partitioner = pt.Partitioner(
    ...
    def cross_match(
        self, othercat=None, n_neighbors=3, dthresh=4.0, client=None, debug=False
    ):
        """
        ...
        assert isinstance(
            othercat, Catalog
        ), "The other catalog must be an instance of hipscat.Catalog."

        cat1_md = self.hips_metadata
        cat2_md = othercat.hips_metadata
        nmatches = 0
        hp_xmatch_map = util.map_catalog_hips(

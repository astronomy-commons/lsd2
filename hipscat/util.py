        ...
        for p in pixs:
            ...
            for mo in mapped_dict:
                ...
                for mp in mapped_pixels:
                    c1_cat_path = os.path.join(
                        cat1_outputdir, f"Norder{o}", f"Npix{p}", "catalog.parquet"
                    )
                    c2_cat_path = os.path.join(
                        cat2_outputdir, f"Norder{mo}", f"Npix{mp}", "catalog.parquet"
                    )
...
def get_csv_urls(
    url="https://cdn.gea.esac.esa.int/Gaia/gdr2/gaia_source/csv/", fmt=".csv.gz"
):
    """

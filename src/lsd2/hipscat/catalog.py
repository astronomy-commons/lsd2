...
from . import lsd2_io
from . import spatial_result as sr
...
class Catalog():
    ...
    def columns(self):
        if self.lsddf is None:
            self.lsddf = self.load()
        
        return self.lsddf.columns
    

    def len(self):
        return int(self.hips_metadata['n_sources'])
    ...
    def cone_search(self, ra, dec, radius, columns=None):
        ...
        for p in pixels_to_query:
            ...
            for mo in mapped_dict:
                ...
                if str(mo) not in cone_search_hpdict.keys():
                    cone_search_hpdict[str(mo)] = []
                cone_search_hpdict[str(mo)].extend(mapped_pixels)
        ...
        self.spatial_result = sr.SpatialResult(
            hipscat = self,
            lsddf = result, 
            healpix_dict=cone_search_hpdict
        ) 
        #calculate the sources with the dask partitinoed ufunc
        return self.spatial_result


    def cross_match(self, othercat=None, c1_cols=[], c2_cols=[], n_neighbors=1, dthresh=0.01, evaluate_margins=True, spatial_filter=False, debug=False):
        '''
            Parameters:
        ...
        # ]
        if spatial_filter and self.spatial_result is not None:
            cat1_hips = self.spatial_result.healpix_dict
        else:
            cat1_hips = cat1_md['hips']

        hc_xmatch_map = util.map_catalog_hips(cat1_hips, self.output_dir,
